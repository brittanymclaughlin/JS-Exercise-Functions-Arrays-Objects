...
sayGoodbye("Hannah");
// it needs to return the value, not just console.log ... like that?
...
function temperatureInF(temperature,unit) {
  ...
  if (unit === "C"){
    ...
    return newTemp + ftemp;
  }
  else if (unit === "F"){
    return temperature + ftemp;
  }
 ...
 * }
*/
function makePersonObject(id, name2, email) {
  const PersonObject = {
  id: id,
  name: name2,
  email: email,
...
function appleIndex(fruits) {
  var AppleTest = fruits.indexOf("apple");
  return AppleTest;
}
...
function getModelYears(inventory) {
  ...
  for (let x = 0; x < inventory.length; x++){
    yearsInv.push(inventory[x].car_year);
  }
  return yearsInv;
}
...
function getOlderCars(inventory,max) {
  const Oldies = []
  for (let x=0;x<inventory.length;x++){
    if (inventory[x].car_year <= max){
      Oldies.push(inventory[x])
    }
  }
  return Oldies;
}
...
function getGermanCars(inventory) {
  const German = []
  for (let x=0;x<inventory.length;x++){
    if ( inventory[x].car_make === "Audi" || 
          inventory[x].car_make ==="Mercedes-Benz" ||
          inventory[x].car_make === "Volkswagen" ||
          inventory[x].car_make === "BMW"){
            German.push(inventory[x]);
          }
      }
  return German;
}
